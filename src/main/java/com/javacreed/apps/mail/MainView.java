...
import com.javacreed.apps.mail.sendtest.SendTestDialog;
import com.javacreed.apps.mail.sendtest.SendTestView;
...
public class MainView extends JFrame implements View {
  ...
  private JComponent createDataComponent() {
    ...
    centerSplitPane.setTopComponent(SwingUtils.wrap(dataTable, "Data"));
    centerSplitPane.setBottomComponent(SwingUtils.wrap(messagePreviewArea, "Preview"));

    final JSplitPane mainSplitPane = new JSplitPane();
    mainSplitPane.setLeftComponent(SwingUtils.wrap(variablesTable, "Variables"));
  ...
  private JComponent createMessageComponent() {
    ...
    panel.add(new JLabel("Template"), constraints);

    templateCombobox = new JComboBox<>(new String[] { "Council Elections 2015", "Easter", "Standard" });
    templateCombobox.setFont(templateCombobox.getFont().deriveFont(14F));
    constraints = new GridBagConstraints();
    constraints.gridx = 1;
    constraints.gridy = row++;
    ...
    constraints.anchor = GridBagConstraints.WEST;
    panel.add(templateCombobox, constraints);
    ...
    constraints.gridx = 2;
    constraints.gridy = 0;
    constraints.gridheight = row;
    constraints.weighty = 1;
    constraints.fill = GridBagConstraints.BOTH;
    panel.add(splitPane, constraints);

    final JList<String> attachmentsList = new JList<>();
    splitPane.setTopComponent(SwingUtils.wrap(attachmentsList, "Attachments"));

    final JList<String> embeddedList = new JList<>();
    splitPane.setBottomComponent(SwingUtils.wrap(embeddedList, "Embedded Images"));
